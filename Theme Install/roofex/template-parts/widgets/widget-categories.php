 ...
 * Custom HTML output
*/
if(!function_exists('roofex_widget_categories_args')){
    add_filter('widget_categories_args', 'roofex_widget_categories_args');
    add_filter('woocommerce_product_categories_widget_args', 'roofex_widget_categories_args');
    function roofex_widget_categories_args($cat_args){
        $cat_args['walker'] = new Roofex_Categories_Walker;
        return $cat_args; 
    }
}

/**
 * Roofex_Categories_Walker
 *
 */
...
class Roofex_Categories_Walker extends Walker_Category {
    /**
    ...
    public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $args['feed_image'] ) || ! empty( $args['feed'] ) ) {
            ...
            if ( empty( $args['feed'] ) ) {
                $alt = ' alt="' . sprintf(__( 'Feed for all posts filed under %s','roofex' ), $cat_name ) . '"';
            } else {
                $alt = ' alt="' . $args['feed'] . '"';
                $name = $args['feed'];
...
/*
if(!function_exists('roofex_woocommerce_layered_nav_term_html')){
    add_filter('woocommerce_layered_nav_term_html', 'roofex_woocommerce_layered_nav_term_html', 10, 4);
    add_filter('woocommerce_layered_nav_count', function (){ return '';});
    
    function roofex_woocommerce_layered_nav_term_html($term_html, $term, $link, $count){
